...
int detect_png(char *filepath, pic_data *out)
{
    ...
    {/*如果是RGB+alpha通道，或者RGB+其它字节*/
        ...
        for(i = 0; i < out->height; i++)
        {
            for(j = 0; j < temp; j += 4)
            {/* 一个字节一个字节的赋值 */
                out->rgba[pos * 4 + 0] = row_pointers[i][j]; // red
                out->rgba[pos * 4 + 1] = row_pointers[i][j+1]; // green
                out->rgba[pos * 4 + 2] = row_pointers[i][j+2];   // blue
                out->rgba[pos * 4 + 3] = row_pointers[i][j+3]; // alpha
                ++pos;
            }
        }
    }
    ...
    {/* 如果是RGB通道 */
        ...
        for(i = 0; i < out->height; i++)
        {
            for(j = 0; j < temp; j += 3)
            {/* 一个字节一个字节的赋值 */
                out->rgba[pos * 3 + 0] = row_pointers[i][j]; // red
                out->rgba[pos * 3 + 1] = row_pointers[i][j+1]; // green
                out->rgba[pos * 3 + 2] = row_pointers[i][j+2];   // blue
                ++pos;
            }
        }
    }
